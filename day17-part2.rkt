...
(define (initial->active state)
  (for*/fold ([active (set)])
    ...
    (if (equal? #\. (2dvector-ref state i j))
        active
        (set-add active (point i j 0 0)))))

(define (active-area active)
  (for/fold ([dims (list 0 0 0 0)])
            ([cube active])
    (let ([new-x (if (> (abs (point-x cube)) (car dims))
          ...
          [new-z (if (> (abs (point-z cube)) (caddr dims))
                     (abs (point-z cube))
                     (caddr dims))]
          [new-w (if (> (abs (point-w cube)) (cadddr dims))
                     (abs (point-w cube))
                     (cadddr dims))])
      (list new-x new-y new-z new-w))))
  ...
  (let ([pocket (find-pocket active)])
    (for*/fold ([new-active active])
               ([i (range (- 0 (car pocket)) (+ 1 (car pocket)))]
                ...
                [w (range (- 0 (cadddr pocket)) (+ 1 (cadddr pocket)))])
      (let ([local_cnt (count-active-neighbors active (point i j k w))]
            [cube_active (set-member? active (point i j k w))])
        (cond [(and cube_active
                    (and (not (equal? 2 local_cnt))
                        (not (equal? 3 local_cnt))))
               (set-remove new-active (point i j k w))]
              [(and (not cube_active)
                    (equal? 3 local_cnt))
               (set-add new-active (point i j k w))]
              [else new-active])))))
